...
- Alterar ```EXTERNAL_LINK_CARDS``` para adicionar links externos que são transformados em cards. Útil pra videos no youtube, conteúdo no drive, etc.
...
- HTML usado no processamento do script **build-site.js** para formular o HTML final de release com os cards de links de material inseridos.
- Mexer como quiser pra moldar o site.
...
- HTML final gerado.
- **Não mexer nele diretamente**, pois ao buildar o projeto novamente, o script vai reescrever este arquivo com base no template e nos materiais.
- Este arquivo que deve ser aberto no navegador para testes, apresentação, etc.

### recursos/icones

- Ícones usados para cards dos materiais.
- Por exemplo, ícone nomeado de "Video.png" pode ser usado pra materiais de extensão .mp4, .mkv, .webm, etc.

### recursos/materiais

- Materiais usados pra criar os cards no site
- Pode ser qualquer extensão, mas nem todas podem ter compatibilidade. Se encontrar problema de combatibilidade ou comportamento inesperado ao tentar abrir uma extensão, script provavelmente precisará de modificação.
...
3. Entre em /dist e abra **index.html** no navegador ou hospede com alguma extensão, como **Live Server** do Vs Code (Altamente recomendado, pois incorpora reload automático da página no navegado)
